...
def create_log(
    ...
    actual_time_naive = body.actual_time.replace(tzinfo=None) if body.actual_time.tzinfo else body.actual_time
    if actual_time_naive > now_utc + timedelta(minutes=1):
        logger.warning(f"[Logs] actual_time 为未来时间: {body.actual_time}, now={now_utc}")
        return Response.error(400, "服药时间不能是未来时间")

    # 计算计划服药时间
    try:
        scheduled_dt = datetime.combine(actual_time_naive.date(), sched.time_point)
    ...
    if existing:
        ...
        try:
            ...
            existing.actual_taken_time = actual_time_naive
    ...
    try:
        log = MedicationLog(
            ...
            actual_taken_time = actual_time_naive,
