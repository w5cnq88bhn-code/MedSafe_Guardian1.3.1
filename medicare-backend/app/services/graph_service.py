...
节点类型：Drug、Disease、Symptom、Patient
关系类型：INTERACTS_WITH、CAUSES、TREATS、CONTRAINDICATED_FOR、HAS_DISEASE、TAKES

二阶推理路径：
  (Drug)-[:CAUSES]->(Symptom)-[:INDICATES]->(Disease)<-[:HAS_DISEASE]-(Patient)
"""
import logging
...
def query_second_order_conflicts(patient_id: int, new_drug_ids: List[int]) -> List[Dict]:
    """
    二阶推理：(新药)-[:CAUSES]->(症状)-[:INDICATES]->(疾病)<-[:HAS_DISEASE]-(患者)
    """
    driver = get_driver()
    if not driver:
        return []
