...
药物冲突检测服务 —— 双引擎架构
================================
引擎一（关系表）：PostgreSQL drug_interactions 表，保证基础冲突检测始终可用。
引擎二（知识图谱）：Neo4j 二阶推理，发现一阶查表无法检测的隐性风险。

两个引擎结果合并，图谱不可用时自动降级为纯关系表模式，系统不中断。

二阶推理示例：
  患者患有"高血压"，新加药物"螺内酯"（保钾利尿剂）
  → 图谱推理：螺内酯 -[:CAUSES]-> 血钾升高 -[:INDICATES]-> 高钾血症
  → 患者若同时服用 ACEI（依那普利），高钾血症风险极高
  → 系统给出"二阶推理警告"，一阶查表无法发现此风险
"""
import logging
...
def _check_conflicts_graph(patient_id: int, new_drug_ids: List[int],
    ...
    引擎二：Neo4j 知识图谱推理。
    包含：
      1. 图谱一阶冲突（与 PG 结果互补，图谱可能有 PG 没有的关系）
      2. 二阶推理：新药→症状→疾病→患者已有疾病
      3. 直接禁忌：新药对患者疾病的直接禁忌
    """
